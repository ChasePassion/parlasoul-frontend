...
* 你的核心目标：
  ...
  * 在项目中注释请使用中文，变量和函数名等请使用英文，需要前端做文案展示的，确保是中文，禁止将中文替换为unicode编码和其他奇怪的字符，如需替换，仅能替换为英文
  * 编辑文件的时候不要改变文件的编码格式，保持UTF-8编码
  * 当用户要求你给出开发方案的时候，你必须全面详细调研代码库，然后针对用户的需求给出一个全面详细的开发方案，确保用户的功能被实现，禁止只实现前端然后使用Mock，禁止只实现接口不调用，一定要端到端实现用户的需求
  * 当用户向你提出系统警告的问题的时候，你需要首先调研代码，研究问题的本质，然后给出1-3个修复方案，不同的修复方案都make sense，不能为了方案数量而去随意提出无意义的显然任何情况下都不会被采用的方案，如果你在调研代码库之后，认为确实没有更多方案了，那么只提出数量合适的不同修复方案即可,当用户要求你修复问题的时候，你必须直击问题本质，按照修复方案把问题修复了，而不是把警告关闭
  * 使用Langchain_learn的conda虚拟环境启动后端，运行后端测试
  * 优先使用shadcn提供的组件
  * 优先考虑使用第八部分提到的工具
  * 完成一个active任务之后把任务放到docs/dev/plan-completed文件夹
  ...
  * plan的phase需要有明确的定义和边界，且phase需要递进，在边界里面的添加到这个phase，不在这个边界里面的重新开一个phase
  * 你应该首先使用本文档里面第八部分的工具
  * 编写文档的时候尽量避免评论式的内容，例如：“xxx说错了，xx才是对的”，“xxx过时了，xxx才是最新的”
  * 仅完成用户提到的修改
  * 当用户没有特别要求的时候，工作范围限定在cwd
  * 当用户向你提出一个bug的时候，你需要获取足够多的信息，假设问题，阅读代码，思考，搜索网络，验证问题，当你认为信息不够需要更多信息的时候，你可以向用户寻求帮助，尤其当这个问题属于网络请求，前端问题的时候，更应该要求用户使用devtool帮助你修复问题
  * 尽量避免使用fallback把错误吞了，更好的做法是传递明确的错误信息
  * 当用户要求你修复问题的时候，你应该首先考虑这个问题是否有必要存在，这个问题是否是另一个大问题的派生，如果是可能应该先解决大问题，先改问题定义，先确认问题等等，如果不是考虑如何解决这个问题
  * 当用户给你一个需求的时候，你需要首先讲述你对这个需求的理解，让用户审阅你是否真的理解了用户的需求，同时澄清用户的需求，当用户确认之后，你需要阅读当前代码，了解一下当前的代码实现，然后再开始制定计划
---
